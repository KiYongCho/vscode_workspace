...
console.log(arr1.some(ele => !(ele%5)) ? '5의 배수 있음' : '5의 배수 없음');

// 4. 배열의 모든 요소가 짝수인지 확인
console.log(arr1.every(ele => !(ele%2)) ? '모두 짝수' : '모두 짝수는 아님');
...
// 메소드 체이닝(method chainning) : 메소드의 수행결과를 다음 메소드의 입력으로 연결
console.log(
...
console.log(
    persons.filter(person => person.address.si=='서울시')
...
// 10. 각 사람의 거주지 주소 중 시이름에서 '시', 동이름에서 '동'을 빼고 이름, 나이, 주소를 출력
//      ex) 홍길동, 20세, 서울 역삼
// * substring 함수를 찾아서 해보세요!
//    substring(시작인덱스, 끝인덱스+1) : 문자열의 시작인덱스에서 끝인덱스까지의 부분문자열
//                                                       ex) 'Hello'의 substring(1, 4) => 'ell'
persons.forEach(
    person => console.log(
        person.name + ', ' +
        person.age + '세, ' +
